...
from .learning_rate_estimator import LearningRateEstimator
...
def train_model(model_source, model_dest, initial_batch_size=256, initial_lr=0.0003, gpu=True, force_lr=False, lr_multiplier=1):
    ...
    with tf.device(device):
        return train_model_v5(model_source, model_dest, initial_batch_size, initial_lr, gpu, force_lr, lr_multiplier)


def train_model_v5(model_source, model_dest, initial_batch_size=256, initial_lr=0.0003, gpu=True, force_lr=False, lr_multiplier=1):
    ...
    # lr_estimator = LearningRateEstimator(model_meta)
    # lr_estimator.start_training_thread()

    optimizer = training_manager.get_optimizer()
    model.compile(optimizer=optimizer, loss='categorical_crossentropy',
                  metrics=['categorical_crossentropy'])

    def lr_sheduler(prev_lr):
        next_lr_tm = training_manager.get_next_lr(prev_lr)
        print('next_lr_tm', next_lr_tm)

        # next_lr_ai = lr_estimator.get_next_learning_rate()
        # print('next_lr_ai', next_lr_ai)

        return next_lr_tm

        # return (next_lr_tm+next_lr_ai)/2

    lr_scheduler_callback = LearningRateScheduler(lr_sheduler)
    ...
    while True:
        ...
        # lr_estimator.after_epoch(loss=val, lr=model.optimizer.lr.numpy())
