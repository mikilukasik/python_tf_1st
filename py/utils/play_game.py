...
if __name__ == "__main__":
    ...
    # Play Engines against Stockfish 50 times each
    for engine in engines:
        for idx in range(50):  # 50 games in total
            if idx % 2 == 0:
                result = tournament.play_game(engine, engine_sf)
                key = f"{engine.name} vs Stockfish"
                # For individual game logging
                individual_key = f"{engine.name} vs Stockfish Game {idx+1}"
            else:
                result = tournament.play_game(engine_sf, engine)
                key = f"Stockfish vs {engine.name}"
                # For individual game logging
                individual_key = f"Stockfish vs {engine.name} Game {idx+1}"
            ...
            if key in tournament.results:
                if result == WHITE_WIN:
                    tournament.results[key][0] += 1
                elif result == BLACK_WIN:
                    tournament.results[key][1] += 1
                else:
                    # Increment the number of draws by 1
                    tournament.results[key][2] += 1
            else:
                if result == WHITE_WIN:
                    tournament.results[key] = [1, 0, 0]
                elif result == BLACK_WIN:
                    tournament.results[key] = [0, 1, 0]
                else:
                    # Initialize with one draw
                    tournament.results[key] = [0, 0, 1]

            tournament._log_current_ranking()
