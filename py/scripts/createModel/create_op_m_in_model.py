...
# load another model that will also be used in the merged model
inpconv_model_name = '../models/inpConv_c16x2x6_skip_l2_d101010_l1_do1_bn/1.6967400099549974'
inpconv_model = load_model(inpconv_model_name)

# for layer in progress_model.layers:
#     layer.trainable = False

for layer in inpconv_model.layers:
    layer.trainable = False
...
inpconv_model._name = 'inpconv_model'
opening_model._name = 'opening_model'
mid_end_model._name = 'mid_end_model'

voting_model = create_champ_model(filter_nums=[16, 32, 64, 128, 256],
...
output_1 = opening_model(input_tensor)
output_2 = mid_end_model(input_tensor)
output_3 = inpconv_model(input_tensor)
ratios = voting_model(input_tensor)
...
save_model(merged_model, '../models/op_midend_inpconv/_orig')
