...
with tf.device('/cpu:0'):
    ...
    model.compile(optimizer=tf.keras.optimizers.legacy.Adam(learning_rate=0.0003), loss='categorical_crossentropy',
    ...
    def saveModel(model, avg, lastSavedAvg, qName):
        model.save('./models/c2d2_cG_v1/' + qName + '_' + str(avg))
        print('model saved.    * * * * * * * * * * * * * * * * * * * * * * * *')
        print('                * * * * * * ', qName, avg, ' * * * * * * ')
        print('                * * * * * * * * * * * * * * * * * * * * * * * *')

        if lastSavedAvg < 9999:
            shutil.rmtree(r'./models/c2d2_cG_v1/' +
