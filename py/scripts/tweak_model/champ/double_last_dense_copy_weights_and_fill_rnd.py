...
def extend_weights(_weights, target_shape=(1920, 1837)):
    ...
    # Pad the weights with random values for each row of padding
    # padded_weights = np.pad(
    #     weights, pad_width, mode='constant', constant_values=1000)

    padded_weights = np.pad(weights, pad_width, mode='median')

    return [padded_weights, _weights[1]]


def extend_weights_2(w):
    return [np.concatenate([w[0], np.random.rand(*w[0].shape)], axis=1), np.concatenate([w[1], np.random.rand(*w[1].shape)])]
...
def copy_weights(old_layer, new_layer, new_input_units, weight_extender=extend_weights_2):
...
save_model(new_model, '../models/champ_he_M_d1024-d1024_vr1/_blank', model_meta)
